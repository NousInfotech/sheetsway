...
export default function FeaturesPortal({
  ...
  const [currFeature, setCurrFeature] = useInterval(0, features.length);
  ...
  return (
    <div className="min-h-screen flex flex-col items-center pb-10 container mx-auto">
      ...
      <div className="flex flex-col md:flex-row items-center space-x-10 gap-60">
        <FadeUpAnimation
          className="relative bg-white rounded-[80px] hidden md:block w-60 shadow-[0px_0px_3px_rgba(255,140,0,0.1)] p-5"
          duration={0.5}
        >
            ...
            <div className="w-[1.6px] bg-gray-200 h-36 relative">
              <div
                style={{
                  ...
                  backgroundColor: `hsl(30, 100%, ${80 - currFeature * 4}%)`,
                }}
            ...
            >
              {features[currFeature].boxText.map((text, index) => (
                <motion.li
                  ...
                  key={index}
                  className="grid grid-cols-[1rem_1fr] gap-2 items-center justify-center space-x-4 px-[10px] rounded-md p-1 border border-orange-50 shadow-[0px_0px_3px_rgba(255,140,0,0.18)] "
                >
        ...
        <FadeUpAnimation
          duration={1.2}
          className="grid grid-cols-1 md:grid-cols-2 gap-6 gap-x-14 lg:max-w-2xl"
        >
